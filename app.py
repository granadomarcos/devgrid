...
token = "746b57df577be4b85706eebc87f77825"
...
async def get(
    ...
    **kwargs
    ...
    url = f"https://api.openweathermap.org/data/2.5/group?id={color}&units=metric&appid={token}"
    ...
    db.session.commit()
    completed = db.session.query(func.count(WeatherInfo.user_id)).scalar()
    progress = (completed / len(colors)) * 100
